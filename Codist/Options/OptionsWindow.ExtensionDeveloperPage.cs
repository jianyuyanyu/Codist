	...
	sealed class ExtensionDeveloperPage : OptionPageFactory
	{
		...
		sealed class PageControl : OptionPage
		{
			...
			static ThemedButton MakeToolButton(ExporterBase exporter) {
				return new ThemedButton(exporter.IconId, exporter.Title, exporter.Title + "\n" + exporter.Description, exporter.Export) { Margin = WpfHelper.SmallMargin };
			}
		}
		...
		sealed class ClassificationFormatMapExporter : ExporterBase
		{
			public static readonly ClassificationFormatMapExporter Instance = new();

			ClassificationFormatMapExporter() { }

			public override int IconId => IconIds.CustomizeStyle;
			public override string Title => R.T_ExportClassificationFormatMap;
			public override string Description => R.T_ExportClassificationFormatMapTip;
			protected override string DefaultFileName => "ClassificationFormatMap.txt";
			...
			protected override void ExportContent(StreamWriter writer) {
				...
				writer.WriteLine("Name\tBaseTypes");
				foreach (var item in cfm.CurrentPriorityOrder) {
					if (item is null) {
						continue;
					}
					writer.Write(item.Classification ?? "?");
					writer.Write('\t');
